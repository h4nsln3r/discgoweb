...
export default async function DiscsWorthlessPage() {
  ...
  return (
    <main className="p-4 sm:p-6 max-w-3xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-stone-100">Värdelösa discar</h1>
          <p className="text-stone-400 text-sm mt-1">
            Discar som användare har markerat som värdelösa. Klicka på en disc eller profil för att se mer.
          </p>
        </div>
        <Link
          href="/discs"
      ...
      {items.length === 0 ? (
        <div className="rounded-2xl border border-retro-border bg-retro-surface p-8 text-center">
          <p className="text-stone-400">Inga discar markerade som värdelösa just nu.</p>
        ...
        <ul className="space-y-3">
          {items.map((row) => (
            ...
            >
              ...
              >
                <span>Värdelös enligt: {row.profiles?.alias?.trim() || "Anonym"}</span>
